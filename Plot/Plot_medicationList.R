...
dqa.check.names <- sort(unique(df.MedicationList$dqaName))
...
df.MedicationList.info <- data.frame(matrix(ncol = length(c.name),
                                       nrow = length(r.name),
...
for(i in 1:length(dqa.check.names)){
  ...
  rows <- grep(name, df.MedicationList$dqaName)
  ...
  if(length(rows) == 2){
    ...
    df.MedicationList.info <- rbind(df.MedicationList.info, df.info)
  }
...
df.MedicationList.mv <- df.MedicationList.info %>%
...
df.MedicationList.mv$mv_name <- factor(df.MedicationList.mv$mv_name, levels = sort(unique(as.character(df.MedicationList.mv$mv_name)), decreasing = TRUE))

#create df for overall statistic for missing values
s.true <- sum(df.MedicationList.mv$count[grep("true", df.MedicationList.mv$text)])
s.false <- sum(df.MedicationList.mv$count[grep("false", df.MedicationList.mv$text)])
total <- s.true + s.false

df.MedicationList.mv.overall <- data.frame("sumTrue" = s.true,
                                      "percentTrue"= round((s.true/total*100), digits = 0),
                                      "sumFalse" = s.false,
                                      "percentFalse"= round((s.false/total*100), digits = 0),
                                      "total" = total,
                                      row.names = "MedicationList")

#create df for remain DQ check in df.MedicationList.info
df.MedicationList.rest <- df.MedicationList.info %>%
...
df.MedicationList.rest$rest_name <- factor(df.MedicationList.rest$rest_name, levels = sort(unique(as.character(df.MedicationList.rest$rest_name)), decreasing = TRUE))

#create df containing inadmissible categorical values
df.MedicationList.icv <- df.MedicationList.info %>%
...
df.MedicationList.icv$icv_name <- factor(df.MedicationList.icv$icv_name, levels = sort(unique(as.character(df.MedicationList.icv$icv_name)), decreasing = TRUE))

################################################################################
# plot: missing values blue hue
################################################################################

p.medList.mv <- ggplot(df.MedicationList.mv, aes(x = mv_name, y = count, fill = text)) + 
