...
# Script purpose:
# 1) create plots related to the procedure resource
# Notes:
...
df.Procedure.info <- data.frame(matrix(ncol = length(c.name),
                                       nrow = length(r.name),
...
for(i in 1:length(dqa.check.names)){
  ...
  rows <- grep(name, df.Procedure$dqaName)
  ...
  if(length(rows) == 2){
    ...
    df.Procedure.info <- rbind(df.Procedure.info, df.info)
  }
...
df.Procedure.mv <- df.Procedure.info %>%
...
df.Procedure.mv$mv_name <- factor(df.Procedure.mv$mv_name, levels = sort(unique(as.character(df.Procedure.mv$mv_name)), decreasing = TRUE))

#create df for overall statistic for missing values
s.true <- sum(df.Procedure.mv$count[grep("true", df.Procedure.mv$text)])
s.false <- sum(df.Procedure.mv$count[grep("false", df.Procedure.mv$text)])
total <- s.true + s.false

df.Procedure.mv.overall <- data.frame("sumTrue" = s.true,
...
#create df for remain DQ check in df.Procedure.info
df.Procedure.rest <- df.Procedure.info %>%
...
df.Procedure.rest$rest_name <- factor(df.Procedure.rest$rest_name, levels = sort(unique(as.character(df.Procedure.rest$rest_name)), decreasing = TRUE))

#create df containing inadmissible categorical values
df.Procedure.icv <- df.Procedure.info %>%
...
df.Procedure.icv$icv_name <- factor(df.Procedure.icv$icv_name, levels = sort(unique(as.character(df.Procedure.icv$icv_name)), decreasing = TRUE))

################################################################################
# plot: missing values blue hue
################################################################################

p.pro.mv <- ggplot(df.Procedure.mv, aes(x = mv_name, y = count, fill = text)) + 
  ...
  scale_y_continuous(#"Gesamtzahl der Fälle pro Procedure",
    limits = c(0, floor(max(df.Procedure.mv$count)/10)*10+5),
    #minor_breaks = NULL,
    breaks = seq(0, floor(max(df.Procedure.mv$count)/10)*10+5, 5)
  )

#p.pro.mv

################################################################################
# plot: df.Procedure.rest values
################################################################################

#p.pro.rest <- ggplot(df.Procedure.rest, aes(x = rest_name, y = count, fill = text)) + 
#  geom_bar(stat = "identity") + coord_flip() +
#  
...
#  
#  scale_y_continuous(#"Gesamtzahl der Fälle pro Procedure",
#    limits = c(0, floor(max(df.Procedure.rest$count)/1000)*1000+1000),
#    #minor_breaks = NULL,
#    breaks = seq(0, floor(max(df.Procedure.rest$count)/1000)*1000+1000, 1000) #make sure this is the same sequence as .mv
#  )
...
#use package cowplot
#p.pro.grid <- plot_grid(p.pro.mv, p.pro.rest,
