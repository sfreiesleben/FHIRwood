...
# Script purpose:
# 1) create plots related to the medication administration resource
# Notes:
...
dqa.check.names <- sort(unique(df.MedicationAdministration$dqaName))
...
for(i in 1:length(dqa.check.names)){
  ...
  rows <- grep(name, df.MedicationAdministration$dqaName)
  ...
  if(length(rows) == 2){
    ...
    df.MedicationAdministration.info <- rbind(df.MedicationAdministration.info, df.info)
  }
...
df.MedicationAdministration.mv <- df.MedicationAdministration.info %>%
...
df.MedicationAdministration.mv$mv_name <- factor(df.MedicationAdministration.mv$mv_name, levels = sort(unique(as.character(df.MedicationAdministration.mv$mv_name)), decreasing = TRUE))

#create df for overall statistic for missing values
s.true <- sum(df.MedicationAdministration.mv$count[grep("true", df.MedicationAdministration.mv$text)])
s.false <- sum(df.MedicationAdministration.mv$count[grep("false", df.MedicationAdministration.mv$text)])
total <- s.true + s.false

df.MedicationAdministration.mv.overall <- data.frame("sumTrue" = s.true,
                                      "percentTrue"= round((s.true/total*100), digits = 0),
                                      "sumFalse" = s.false,
                                      "percentFalse"= round((s.false/total*100), digits = 0),
                                      "total" = total,
                                      row.names = "MedicationAdministration")

#create df for remain DQ check in df.MedicationAdministration.info
df.MedicationAdministration.rest <- df.MedicationAdministration.info %>%
...
df.MedicationAdministration.rest$rest_name <- factor(df.MedicationAdministration.rest$rest_name, levels = sort(unique(as.character(df.MedicationAdministration.rest$rest_name)), decreasing = TRUE))

#create df containing inadmissible categorical values
df.MedicationAdministration.icv <- df.MedicationAdministration.info %>%
...
df.MedicationAdministration.icv$icv_name <- factor(df.MedicationAdministration.icv$icv_name, levels = sort(unique(as.character(df.MedicationAdministration.icv$icv_name)), decreasing = TRUE))

################################################################################
# plot: missing values blue hue
################################################################################

p.medAdmin.mv <- ggplot(df.MedicationAdministration.mv, aes(x = mv_name, y = count, fill = text)) + 
