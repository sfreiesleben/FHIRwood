...
# Script purpose:
# 1) create plots related to the medication request resource
# Notes:
...
dqa.check.names <- sort(unique(df.MedicationRequest$dqaName))
...
df.MedicationRequest.info <- data.frame(matrix(ncol = length(c.name),
                                       nrow = length(r.name),
...
for(i in 1:length(dqa.check.names)){
  ...
  rows <- grep(name, df.MedicationRequest$dqaName)
  ...
  if(length(rows) == 2){
    ...
    df.MedicationRequest.info <- rbind(df.MedicationRequest.info, df.info)
  }
...
df.MedicationRequest.mv <- df.MedicationRequest.info %>%
...
df.MedicationRequest.mv$mv_name <- factor(df.MedicationRequest.mv$mv_name, levels = sort(unique(as.character(df.MedicationRequest.mv$mv_name)), decreasing = TRUE))

#create df for overall statistic for missing values
s.true <- sum(df.MedicationRequest.mv$count[grep("true", df.MedicationRequest.mv$text)])
s.false <- sum(df.MedicationRequest.mv$count[grep("false", df.MedicationRequest.mv$text)])
total <- s.true + s.false

df.MedicationRequest.mv.overall <- data.frame("sumTrue" = s.true,
                                      "percentTrue"= round((s.true/total*100), digits = 0),
                                      "sumFalse" = s.false,
                                      "percentFalse"= round((s.false/total*100), digits = 0),
                                      "total" = total,
                                      row.names = "MedicationRequest")

#create df for remain DQ check in df.MedicationRequest.info
df.MedicationRequest.rest <- df.MedicationRequest.info %>%
...
df.MedicationRequest.rest$rest_name <- factor(df.MedicationRequest.rest$rest_name, levels = sort(unique(as.character(df.MedicationRequest.rest$rest_name)), decreasing = TRUE))

#create df containing inadmissible categorical values
df.MedicationRequest.icv <- df.MedicationRequest.info %>%
...
df.MedicationRequest.icv$icv_name <- factor(df.MedicationRequest.icv$icv_name, levels = sort(unique(as.character(df.MedicationRequest.icv$icv_name)), decreasing = TRUE))

################################################################################
# plot: missing values blue hue
################################################################################

p.medRe.mv <- ggplot(df.MedicationRequest.mv, aes(x = mv_name, y = count, fill = text)) + 
...
################################################################################
# plot: df.MedicationRequest.rest values
################################################################################

p.medRe.rest <- ggplot(df.MedicationRequest.rest, aes(x = rest_name, y = count, fill = text)) + 
  geom_bar(stat = "identity") + coord_flip() +
  ...
  scale_fill_manual(labels = c("Cancelled", "Active", "Option", "Order"), #make sure the order is correct here!
                    values = c("#D8EAF4","#9ECAE2", "#32779E", "#1A3E52")) +
...
#use package cowplot
p.medRe.grid <- plot_grid(p.medRe.mv, p.medRe.rest,
