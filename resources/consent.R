...
#get stratifier
stratifier <- unlist(j.Consent$group$stratifier , recursive = FALSE)
  ...
  for(k in 1:nrow(df.stratum)){
    ...
    if(is.null(stratum.text)){
      ...
      for(l in 1:nrow(df.stratum.value)){
        dqa.coding <- paste(unlist(df.stratum.value$coding[l], use.names = FALSE), collapse = ";")
        dqa.population <- unlist(df.stratum$population[l], use.names = FALSE)[3]
        
        
        info <- c(dqa.coding, dqa.population)
        
        df.info <- data.frame(info, row.names = c("text", "count"))
        colnames(df.info) <- paste0(stratifier.code[j], ".", k)
